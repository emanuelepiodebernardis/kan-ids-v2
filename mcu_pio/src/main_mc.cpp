...
void setup() {
  ...
  for (uint8_t w = 0; w < N_WARM; w++) {
    for (uint8_t i = 0; i < 10; i++) x[i] = TV_RD16(KMCTV_X[w][i]);
    for (uint8_t j = 0; j < 4; j++)  c[j] = TV_RD8(KMCTV_CAT[w][j]);
    sink ^= kan14_mc_predict(x, c);
  }
  ...
  for (uint16_t r = 0; r < N_RUNS; r++) {
    ...
    for (uint8_t i = 0; i < 10; i++) x[i] = TV_RD16(KMCTV_X[k][i]);
    for (uint8_t j = 0; j < 4; j++)  c[j] = TV_RD8(KMCTV_CAT[k][j]);
    uint32_t t0 = micros();
    uint8_t p = kan14_mc_predict(x, c);
    uint32_t dt = micros() - t0;
    uint8_t exp_p = TV_RD8(KMCTV_EXPECTED[k]);
    uint8_t lab   = TV_RD8(KMCTV_LABEL[k]);
    if (p == exp_p) n_ok++;
    if (p == lab)   n_match_label++;
    sum += dt; sum2 += (float)dt * dt;
    if (dt < tmin) tmin = dt;
    if (dt > tmax) tmax = dt;
    Serial.print(F("mc_coeff_int8,")); Serial.print(r); Serial.print(',');
    Serial.print(F("mc")); Serial.print(',');
  ...
  Serial.print(F("SUMMARY variant=mc_coeff_int8 n=")); Serial.print(N_RUNS);
