...
\urlstyle{same}
\hypersetup{pdftitle={Integer Kolmogorov-Arnold Networks for IoT Intrusion Detection},pdfauthor={Oleksandr Kuznetsov; Emanuele Pio De Bernardis; Emanuele Frontoni},pdfsubject={Paper 1 IEEE Access working manuscript v0.12.0}}
...
\newcommand{\kanidsdraftfoot}{\hbox to\textwidth{{\footervolfont Working manuscript, version 0.12.0}\hfill{\footerpagefont\thepage}}}
...
\begin{abstract}
We examine whether a sampled lookup table preserves the decisions of a frozen additive integer Kolmogorov--Arnold network and what implementation cost this representation change entails on two microcontrollers. Exhaustive enumeration includes the exact rounding of each finite-domain edge function; signed error intervals give sufficient agreement certificates relative to the integer coefficient evaluator. A 20,554-byte table certifies 42,206 of 42,209 prepared test decisions, with empirical agreement on all decisions; the coefficient arrays occupy 254 bytes. A controlled ESP32-C3 experiment compares representation, code placement and parameter placement under a common timing boundary. With code in Flash and parameters in DRAM, the LUT is about 1.71 times faster. Separate physical USB measurements support whole-board engineering estimates for a sustained workload that repeatedly cycles twenty prepared inputs; estimated energy per call decreases by about 84\% on Mega 2560 and 37\% on ESP32-C3. The experiments exclude live preprocessing and do not measure peak runtime memory. Historical classification results and duplicate-sensitivity analysis delimit the evaluated detector rather than establish architectural superiority. The contribution is an auditable integer-export analysis and physical implementation case study, with explicit limits on generalization across workloads and networks.
\end{abstract}
\begin{keywords}
...
The v0.12.0 release map, \path{evidence/ARTIFACT_MAP.md} (repository copy: \path{evidence/review_v012/ARTIFACT_MAP.md}), links each central claim to its input hashes, analysis command and expected outputs. It distinguishes arithmetic replay on prepared inputs, reaggregation of historical metrics, conditional preprocessing recovery and physical-measurement provenance. The new exact-row subgroup analysis evaluates frozen inputs without fitting. A complete fitted raw-flow-to-decision pipeline is not established for arbitrary new traffic. The source CSVs, supplied caches, analysis inputs and scripts are identified by hashes in the accompanying verification package. The recovered UNSW CSVs came from a public mirror and match the hashes reported for the historical files; hash equality does not independently establish which bytes the original fitted processes consumed. Manuscript archives contain the LaTeX sources, bibliography, figures and build scripts. Public archival deposition of the revised software and physical records remains pending; the public baseline must not be mistaken for that revised release. Dataset use remains subject to the original providers' terms.
...
\section{Retained USB-Energy Numerical Precision}
\label{app:energy-details}
The following table preserves the original reported means for audit against the full-precision CSV. These values carry no implied absolute accuracy beyond the instrument and protocol evidence in Section~\ref{sec:energy-pilots}.
