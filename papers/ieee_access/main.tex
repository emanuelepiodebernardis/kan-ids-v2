...
\urlstyle{same}
\hypersetup{pdftitle={Integer Kolmogorov-Arnold Networks for IoT Intrusion Detection},pdfauthor={Oleksandr Kuznetsov; Emanuele Pio De Bernardis; Emanuele Frontoni},pdfsubject={Paper 1 IEEE Access working manuscript v0.11.0}}
...
\newcommand{\kanidsdraftfoot}{\hbox to\textwidth{{\footervolfont Working manuscript, version 0.11.0}\hfill{\footerpagefont\thepage}}}
...
\pagestyle{headings}
\begin{document}
\history{Working manuscript, 15 September 2026.}
\doi{}
\title{Integer Kolmogorov--Arnold Networks for IoT Intrusion Detection: Decision Certificates and Memory--Latency--Energy Trade-offs}
...
\begin{abstract}
We study integer coefficient and sampled lookup-table representations of a frozen additive Kolmogorov--Arnold network for intrusion detection. The coefficient arrays occupy 254 bytes and the selected table 20,554 bytes. Exhaustive enumeration of each edge's finite Q12 domain includes the implemented integer rounding. Signed edge-error extrema give sufficient certificates for threshold agreement, certifying 42,206 of 42,209 prepared test decisions; all decisions agree empirically. A smaller 10,314-byte table leaves twelve decisions uncertified, compared with three for the selected table. This retrospective comparison retains the training-only fallback selection and does not establish a minimum-memory solution. Physical 500-flow kernel experiments give a 5.746-fold LUT speedup on Arduino Mega 2560. A controlled ESP32-C3 factorial experiment gives a 1.706-fold speedup with Flash code and DRAM parameters and identifies a code--parameter placement interaction. A separate USB-energy pilot uses five implementations, two acquisitions per implementation and board, and repeated batches of twenty prepared source flows. Whole-board energy estimates for coefficient and LUT calls are 492.62 and 79.64 microjoules on Mega and 0.8747 and 0.5516 microjoules on ESP32-C3. These descriptive estimates use measured central-window power and MCU batch duration; they exclude preprocessing and do not resolve individual calls. Retrospective cross-dataset stress tests show poor performance. Source-to-cache checks and reconstructed training cohorts identify changes in category--label associations without establishing their contribution to historical errors. The contribution is an auditable agreement analysis and measured memory--latency--energy trade-off, not universal detection superiority or unconditional lossless compilation. Peak runtime memory remains unmeasured.
\end{abstract}
\begin{keywords}
...
\item A physical comparison of five frozen implementations on Mega 2560 and ESP32-C3 using 500 common source observations, followed by a controlled ESP32-C3 factorial experiment that separates representation, code placement and parameter placement. A separate twenty-acquisition pilot quantifies whole-board USB energy for sustained batches on the same two boards.
\item An evidence-linked assessment of exact additive explanations and retrospective cross-dataset stress tests. It distinguishes model quality from compilation agreement and checks source-to-cache integrity and source--target associations under the retained joint-sampling protocol.
\end{enumerate}

The classification and deployment experiments serve complementary purposes. The device kernels implement native TON\_IoT models; they are not the harmonized joint-source classifiers. No adaptation, drift-recovery method, new architecture search, or new training cycle is introduced here. The finalization stage corrects selection and reporting, compiles fixed representations, and extends the physical evidence.
...
representations on AVR and RV32 microcontrollers. The contribution is a
verified deployment study and its reproducible arithmetic and hardware
evidence, rather than a new quantization principle or an unconditional
guarantee of lossless compilation.
...
Our earlier study~\cite{debernardis2026lightweight} already reported quantized conventional classifiers and physical Mega/ESP32-C3 deployment. It did not include KANs. Its timing experiment used two distinct input vectors, each repeated 250 times. Our short-kernel experiment uses 500 distinct source flows shared across implementations, with five reset-separated passes. The separate energy pilot repeatedly cycles twenty prepared source flows to obtain sustained power windows; its amortized times are reported separately. This study also evaluates the specific harmonized and joint-source protocols described below. These changes extend the model and input coverage; they do not make the old and new latency numbers directly comparable. Model fits, representations, feature adapters, and timing cohorts differ, so we do not calculate an optimization speedup against that earlier article.
...
We distinguish three evidence layers. The dataset-scale model fits and their run-level metrics are preserved outputs of the original experimental stage. During finalization, selected means and sample standard deviations were independently recomputed from those records, without rerunning the fits. The integer coefficient/LUT calculations and explanation sums were independently recomputed from fixed headers and prepared arrays. Finally, physical measurements were collected on the two boards and checked against archived serial traces and build artifacts. Agreement at one layer is not used to claim independent replication at another.
...
Numerical preprocessing coerces values, imputes zero, ranks candidates by MI on at most 40,000 training rows, and selects the required numerical inputs. Selected skewed fields receive $\log(1+\max(v,0))$. A training-fitted normal-output quantile transform, requesting at most 1,000 quantiles, is clipped to $[-3.5,3.5]$. Its subsampling follows the recorded library version. Category vocabularies are fitted on training values, with index zero reserved for unseen categories. An unknown slot is represented explicitly; its contribution is not assumed to be zero. Native TON MI uses the multiclass attack-type label even for binary prediction, whereas harmonized MI uses the binary label.
...
\caption{Three predetermined illustrative cases from the historical 200-vector fixture. Bars are all fourteen uncentered integer contributions; positive values increase the attack score and negative values decrease it. True and predicted classes are shown explicitly. Numerical feature values are transformed coordinates, and case selection is descriptive rather than an explanation-quality test.}
\label{fig:xai-local}
\end{figure*}
...
\begin{table*}[t]
\caption{Mean balanced accuracy in the rich harmonized space. The first two columns summarize 50 fits each; the other columns summarize 10 fits each. Joint denotes the equal-domain, normal:attack 1:5 protocol. These conditions differ in training population and sample count.}
\label{tab:cross}
\centering\small
...
The original five-implementation protocol provides the comparison among frozen deployment pipelines. For the ESP32-C3 coefficient--LUT and memory-placement question, the controlled factorial experiment in Section~\ref{sec:c3followup} is the primary comparison. The earlier separate diagnostic is supporting evidence under its own timing boundary. Section~\ref{sec:energy-pilots} adds a separate sustained-batch USB-energy pilot; its twenty-flow repeated workload is not pooled with any of the 500-flow experiments.

The devices are an Arduino Mega 2560 R3 with a 16-MHz ATmega2560 and an ESP32-C3 board with a SuperMini layout. The ESP probe identified revision 0.4, 4 MB of embedded Flash, and the firmware reported a 160-MHz CPU clock. Both boards were USB powered; Fig.~\ref{fig:mcu_boards} shows the physical boards used in the measurements. Builds used PlatformIO Core 6.1.19. The Mega toolchain records atmelavr 5.1.0, Arduino AVR 5.2.0, and GCC 7.3.0. ESP builds use espressif32 6.12.0, Arduino 2.0.17, and GCC 8.4.0; the effective final optimization option is \texttt{-Os}. Recorded commands and binaries define the measured configurations.
...
\subsection{Latency and replay consistency}
\begin{table}[t]
...
\bottomrule
\end{tabular}
\smallnote{ESP also reserves 48,128 bytes for IRAM code and alignment within the shared SRAM extent; the aliased \texttt{.dram0.dummy} range is counted once. A separate 32-byte RTC allocation is excluded from the shared SRAM extent. Stack and heap peaks are unmeasured on both boards.}
\end{table*}
...
Federated training remains a possible extension of this work. Under shared preprocessing, fixed basis functions, and a common initialization, the floating additive KAN provides aligned coefficient vectors for model averaging~\cite{mcmahan2017fedavg}; quantization and LUT generation would follow aggregation. This operation does not directly apply to independently grown trees with different topologies, which require other collaboration mechanisms. In CO-DEFEND, the receiving participant evaluates candidate trees on its own local validation data, rather than accepting a sender's reported accuracy. For DT, it selects one complete tree; for RF, it pools individual trees from local and received forests and retains the top-ranked trees up to a prescribed ensemble size~\cite{cajaraville2026codefend}. The protocol assumes benign participants and does not demonstrate resistance to model poisoning or backdoors. SecureBoost instead jointly constructs trees under vertical data partitioning with a privileged label holder~\cite{cheng2021secureboost}. Horizontal collaboration also extends beyond model selection and ensemble formation: FederBoost jointly constructs trees by aggregating split statistics through a coordinating participant~\cite{tian2024federboost}. Thus, parameter averaging, ensemble construction, model selection, and aggregation of split statistics are distinct operations. The aligned KAN representation does not establish a more secure federation: standard FedAvg uses a central server, and exposed gradients can leak training examples~\cite{zhu2019deep}. This paper does not establish federated convergence, update privacy, communication cost, training feasibility on the measured microcontrollers, or secure operation without a privileged coordinator.

Several limits constrain inference. Historical test inspection precludes describing these targets as untouched confirmation. Random observation-level splits retain content overlap in the checked canonical TON split and may retain further network-specific dependence. BoT has only 477 normal observations, with 95 in each joint outer test, so normal recall is based on a small minority pool. Repeated seeds reuse observations. Baselines differ in categorical encoding and weighting. The inherited compact configurations are not demonstrated optima. The two feature families used for hardware and transfer are different, and the hardware cohort is balanced and much smaller than the full test population.

The device experiments establish prepared-feature execution on one unit of each board type. The USB pilot adds descriptive whole-board energy estimates under sustained execution; its two acquisitions per model do not establish variation across independent devices, workloads or cold boots. It does not measure live traffic processing, peak runtime SRAM, long-term thermal behavior, or performance under radio and application contention. Instrument accuracy and MCU clock error were not independently calibrated; recording-grid resolution and observed repeatability do not constitute an uncertainty budget. Exact additive explanations describe arithmetic, not causal security mechanisms. These limits delimit the contribution without substituting host or simulator estimates for physical measurements.
...
\end{align}
The $b_r$ are quantized approximations to $6F$ times the usual cubic B-spline basis; the nested floors are part of their definition. Substitution into~\eqref{eq:integer} defines the exact exported score, including negative floors. At $L=1025$, the LUT spacing is eight Q12 units and the per-edge scale exponents are $(7,7,7,6,7,6,7,6,7,6)$. The corresponding exhaustive edge errors are $(889,491,225,327,252,294,597,271,601,324)$, whose sum is 4,271. These finite-domain values are recomputed from the frozen arrays rather than inferred from a floating approximation.
...
\input{inputs/source_target_checks.tex}
\FloatBarrier
...
\phantomsection
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{figures/Kuznetsov.png}}]{Oleksandr Kuznetsov}
...
\noindent\begin{minipage}{\columnwidth}
\phantomsection
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{figures/E_P_Bernardis.png}}]{Emanuele Pio De Bernardis}
...
\noindent\begin{minipage}{\columnwidth}
\phantomsection
\begin{IEEEbiography}[{\includegraphics[width=1in,height=1.25in,clip,keepaspectratio]{figures/fronto.png}}]{Emanuele Frontoni}
