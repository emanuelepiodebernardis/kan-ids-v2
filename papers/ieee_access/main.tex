...
\urlstyle{same}
\hypersetup{pdftitle={Decision-Agreement Certificates for Integer Additive Kolmogorov-Arnold Networks: A Microcontroller Case Study in IoT Intrusion Detection},pdfauthor={Oleksandr Kuznetsov; Emanuele Pio De Bernardis; Emanuele Frontoni},pdfsubject={Paper 1 IEEE Access working manuscript v0.15.3}}
...
\newcommand{\kanidsdraftfoot}{\hbox to\textwidth{{\footervolfont Working manuscript, version 0.15.3}\hfill{\footerpagefont\thepage}}}
...
\def\@IEEEBIOskipN{3\baselineskip} % Keep all biographies and the end marker together.
...
\pagestyle{headings}
\begin{document}
\history{Working manuscript, 19 September 2026.}
\doi{}
\title{Decision-Agreement Certificates for Integer Additive Kolmogorov--Arnold Networks: A Microcontroller Case Study in IoT Intrusion Detection}
...
\tfootnote{}
\markboth{Kuznetsov et al.: Decision-Agreement Certificates for Integer Additive KANs}{Kuznetsov et al.: Decision-Agreement Certificates for Integer Additive KANs}
\begin{abstract}
Exact additive score decomposition makes additive Kolmogorov--Arnold networks suitable for direct arithmetic inspection. We use IoT intrusion detection as a case study of integer export verification and microcontroller implementation. Exhaustive enumeration of finite-domain edge functions includes integer rounding; signed error intervals give sufficient agreement certificates relative to the frozen integer coefficient evaluator. A 20,554-byte table certifies 42,206 of 42,209 prepared test decisions, with empirical agreement on all decisions; the coefficient arrays occupy 254 bytes. The certificates cover coefficient-to-LUT agreement only: export from the original floating model changes 40 test decisions and is not certified. Under a controlled cycle-counter protocol with Flash code and DRAM parameters, the LUT is about 1.71 times faster on ESP32-C3, whereas it is 7.58\% slower under the original five-implementation protocol. A separate sustained workload cycles the same 500 source flows used by the hardware comparison and records whole-board USB energy and batch time under one boundary. Coefficient-to-LUT energy estimates decrease from 498 to 92.2~$\mu$J per iteration on Mega 2560 and from 0.987 to 0.859~$\mu$J on ESP32-C3. Instrumented runs report observed static, stack and heap components, without claiming an exact whole-system memory peak. An endpoint-pair-disjoint floating-model experiment gives additive-KAN balanced accuracy 0.8948 and false-positive rate 20.59\%, both worse than those of the evaluated GAM and MLP baselines. These results support an auditable implementation study while showing the detection limitations of the chosen additive model.
\end{abstract}
\begin{keywords}
...
Kolmogorov--Arnold networks (KANs) represent learned transformations through univariate functions on network edges~\cite{liu2024kan}. Their functions can be evaluated from coefficients or compiled into tables. We select an additive categorical KAN because its output decomposes exactly into numerical and categorical contributions, and its finite-domain edge errors can be exhaustively checked under integer arithmetic. This structure makes it possible to study how export agreement changes with the table memory budget. IoT intrusion detection supplies the application case, with separate classifier comparisons to assess detection quality. A requirement to inspect a separate contribution from every input is different from a requirement to inspect a tree decision path. Both can be useful forms of model inspection; neither establishes that the additive model should replace a more accurate or faster detector. The representation, feature preprocessing, and target processor must be assessed together.
...
The classification and deployment experiments serve complementary purposes. The device kernels implement native TON\_IoT models; they are not the harmonized joint-source classifiers. The endpoint-pair-disjoint study fits separate floating models under a frozen protocol; these fits do not replace the deployed weights or the integer certificate. No adaptation, drift-recovery method or new architecture search is introduced.

\section{Related Work and Scope}
KANs use learnable univariate edge functions~\cite{liu2024kan}, and integer quantization is an established deployment technique~\cite{jacob2018integer}. Earlier LUT-KAN work replaces basis-function evaluation with sampled tables, quantization and integer interpolation on microcontrollers~\cite{kuznetsov2026esl,kuznetsov2026mculut}. The latter study also reports a multilayer CICIDS2017 intrusion detector on physical Mega 2560 and ESP32-C3 boards. The CPU toolkit provides segment-wise quantization and interpolation with NumPy and Numba backends and a DoS case study~\cite{kuznetsov2026toolkit}. Table acceleration, integer interpolation and physical MCU deployment are therefore established starting points for the present study.
...
Hybrid CNN--KAN person detection provides a separate deployment example: Faggi et al. examine quantization strategies and operator conversion for TFLite~\cite{faggi2026quantization}. Their sensor-system study characterizes the effects of runtime, compiler settings and quantization handling on device measurements~\cite{faggi2026sensor}. These results motivate explicit implementation and measurement boundaries; they do not establish agreement or measurement uncertainty for our IDS firmware.
...
The selected model arrays occupy $20L+54=20,554$ bytes, or 80.921 times the coefficient arrays. Post-freeze evaluation gives identical decisions on all 42,209 canonical test rows. Four rows fail the sufficient symmetric margin condition in (\ref{eq:certificate}), although their decisions still agree. The coefficient and LUT confusion counts are $TP=31,313$, $FP=214$, $FN=896$, and $TN=9,786$, yielding $F_1=0.9825844$. These are independently recomputed host integer-model results on preserved transformed arrays, not a physical replay of the whole test set. Earlier test exposure remains disclosed as specified above.
...
\input{inputs/cross_dispersion.tex}

Table~\ref{tab:cross} makes the transfer limitation explicit; Table~\ref{tab:supp-cross-dispersion} reports the corresponding dispersions. Although rich in-domain balanced accuracy is generally high, TON-to-BoT means range from 0.4369 to 0.5573. Additive KAN has the largest observed mean in that direction, whereas MLP16 leads BoT-to-TON with 0.7343. This reversal provides no basis for general KAN superiority. Different sampling mechanisms, feature semantics, and class-conditional distributions remain after harmonization.
...
The original Mega experiment measured a $5.746\times$ coefficient-to-LUT speedup. The controlled ESP32-C3 cycle-counter experiment measured $5.136$ versus $3.010~\mu$s with Flash code and DRAM parameters, a $1.706\times$ speedup. Its factorial contrasts show a code--parameter placement interaction; they do not identify the cause of the earlier protocol discrepancy. All 32,000 timed decisions in that experiment matched the compiled references. The HW500 series pairs whole-board energy with batch time on the common 500-flow cohort: coefficient and LUT estimates are about 498 and 92.2~$\mu$J per iteration on Mega, and 0.987 and 0.859~$\mu$J on ESP32-C3. Separate RAM500 runs pass all reference and positive-control checks and report observed memory components, without an exact whole-system peak. These studies complement the short-kernel observations while preserving distinct measurement boundaries. Additive score decomposition remains exact but is neither uniquely interpretable nor empirically shown to improve analyst decisions. Cross-dataset stress tests show poor performance. Exact raw-to-cache checks and reconstructed source cohorts document category--label association changes on UNSW, while their model-level effects remain unresolved. The result is a reproducible agreement analysis and a measured implementation trade-off, with workload-specific energy and memory observations. It establishes neither superior detection nor universal energy efficiency or worst-case memory use.

\section*{Reproducibility and Data Availability}
The reviewed source is public in the \href{https://github.com/emanuelepiodebernardis/kan-ids-v2}{\texttt{kan-ids-v2}} repository at commit \href{https://github.com/emanuelepiodebernardis/kan-ids-v2/commit/b219fb6671442e2e5dfcda5b6ea49a04d57e7960}{\texttt{b219fb667144}}; it is not a final archival release. The accompanying release candidate retains corrected selection protocols, frozen headers, transformed inputs, common cohort identifiers, run-level metrics, raw hardware archives and independent scripts. Retained analyses cover signed errors, factorial contrasts, hash-matched UNSW/TON source-to-cache verification, reconstructed joint-cohort associations and canonical-split duplicates. Recovered native pre-Q12 inputs, JSONL checkpoints and AVR probe objects support paired-export and provenance checks; verification receipts record identities and independently checked quantities. Hardware records include all twenty original power records and run archives, exact sources, ELF/HEX/BIN files, marker alignment and integrations, audited for provenance, executed loops and energy arithmetic. A manuscript-specific evidence extract links tables to accepted records. The new pair-disjoint study retains fitted preprocessors, floating models, split identities and row-level predictions. Historical joint fitted models, preprocessors and aligned scores remain unavailable.

Artifact maps for v0.15 and historical v0.12 identify input hashes, commands and expected outputs for each central claim, distinguishing prepared-input arithmetic replay, historical-metric reaggregation, conditional preprocessing recovery and physical provenance. The exact-row subgroup analysis uses frozen inputs without fitting. The endpoint-pair study comprises twenty new floating fits, with metrics independently recomputed from 761,740 saved test predictions. HW500 retains two continuous meter records and per-run timing/provenance; RAM500 retains fifteen accepted uploads per board with reference comparisons and instrumentation controls. The release records source versions for each accepted series and retains failed instrumentation attempts as excluded diagnostics. A complete fitted raw-flow-to-decision pipeline for arbitrary new traffic is not established. Verification-package hashes identify source CSVs, caches, inputs and scripts. The UNSW CSVs recovered from a public mirror match the author-reported historical hashes, but this does not establish which bytes the original fitted processes consumed. Manuscript archives include LaTeX sources, bibliography, figures and build scripts. Complete public archival deposition remains pending; the cited commit does not establish availability of every raw record. Dataset use remains subject to the original providers' terms.
...
Table~\ref{tab:energy-pilots-full} preserves the original pilot means for audit against the full-precision CSV. The pilot remains separate from the HW500 study; these digits imply no absolute instrument accuracy.
