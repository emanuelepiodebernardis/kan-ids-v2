# KAN-IDS Paper 1 — English manuscript v0.15.2
...
The existing pilot figure is reproducible with `python3 figures/make_energy_figure.py`. Current paper build, content and visual checks are recorded with the release; old content-check scripts retain historical assertions and do not certify v0.15.2. Verify delivered bytes before rebuilding with `python3 tools/verify_package.py .` when the package manifest is present. A rebuild changes generated files.
