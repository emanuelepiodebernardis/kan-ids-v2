\begin{table*}[t]
\caption{Six distinct ESP32-C3 protocols. All use one physical board at 160 MHz and prepared native TON\_IoT inputs; no row is pooled with another. Placement denotes the inference kernel and model parameters. Retained records establish build settings and observed execution, not hardware-wide guarantees.}
...
Twenty-input pilot (Appendix~\ref{app:energy-details}) & Complete MCU batch duration / call count; central-window USB power & Original pilot Flash parameters; separate linked applications & First 20 flows repeatedly cycled; two acquisitions per model, one boot per C3 pair & Warmup and count calibration before roughly 120 s of active loop; inference, indexing, checksums and interrupts included. Wi-Fi/Bluetooth not initialized; no explicit yield. Coefficient/LUT time ratio: $1.567\times$.\\
HW500 batch (Section~\ref{sec:hw500}) & MCU complete batch / count; USB $VI$ integral over mapped full batch & Separate frozen applications; parameters and input cohort in Flash & Same 500 flows, repeatedly cycled; five technical runs per model & Prepared-row Flash-to-RAM copy, inference, checksum and loop included. Preprocessing, serial and LED markers excluded; 25 accepted runs per board.\\
RAM500 (Section~\ref{sec:ram500}) & Static sections, heap observations and task lifetime watermark; not a latency experiment & Separate instrumented applications & Same 500 inputs; three uploads and three passes per model & Load/predict/reference-check baseline; positive controls afterwards. C3 lifetime watermark already reached before inference; no whole-system peak.\\
\bottomrule
\end{tabular}
\end{table*}
