...
def main():
    ...
    d = prepare14_dict()
    SC = _ART("kcat_smote.npz")
    if os.path.exists(SC):
        ds = np.load(SC, allow_pickle=True)
        Xtr, ymtr, CTtr = ds["Xtr"], ds["ymtr"], ds["CTtr"]
    else:
        from imblearn.over_sampling import SMOTENC
        Xr = d["Xtr"]; CTr = d["CTtr"]; yr = d["ymtr"]
        Xfull = np.hstack([Xr, CTr.astype(np.float64)])
        n_mitm = int((yr == 4).sum())
        sm = SMOTENC(categorical_features=[10, 11, 12, 13],
                     sampling_strategy={4: n_mitm * 10}, random_state=42, k_neighbors=5)
        Xres, yres = sm.fit_resample(Xfull, yr)
        Xtr = Xres[:, :10]; CTtr = Xres[:, 10:].astype(np.int64); ymtr = yres
        np.savez(SC, Xtr=Xtr, ymtr=ymtr, CTtr=CTtr)
        print(f"SMOTENC: MITM {n_mitm} -> {int((yres == 4).sum())}, N={len(yres)}")
    Xtr = (Xtr / 3.5).astype(np.float32)   # [-1,1]
