#!/usr/bin/env python3
"""Variante FOCAL LOSS (gamma=2) di kan_ml_cat_mc: [10 num (Chebyshev) + 4 cat (tabelle)]
...
def main():
    ...
    while st["ep"] < EPOCHS:
        ...
        # focal loss: L = -sw*(1-p_y)^g log(p_y); derivata verificata numericamente:
        #   dL/dz_c = sw * dLdp * p_y * (delta_cy - P_c) / s
        GAMMA = 2.0
        py = np.maximum(P[np.arange(N), ymtr], 1e-9)
        one_m = np.maximum(1.0 - py, 1e-9)
        dLdp = GAMMA * one_m**(GAMMA-1) * np.log(py) - one_m**GAMMA / py
        G = ((sw * dLdp * py)[:, None] * (Y - P)) / s
    ...
    pd.DataFrame([{"modello": f"KAN ml focal g2 num10+cat4 ({HID}h)", "macro_f1": round(mac,4),
                   "weighted_f1": round(wgt,4), "parametri": n_par,
                   "riferimento_lgbm": 0.9675}]).to_csv("results/kan_ml_cat_focal_real.csv", index=False, lineterminator="\n")
    pickle.dump(st, open(CK, "wb"))
    print("DONE")
