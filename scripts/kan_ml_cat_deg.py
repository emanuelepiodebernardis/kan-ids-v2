...
def main():
    ...
    if os.path.exists(CK):
        ...
        params = [ (rng.randn(K, HID, DEG+1)*0.1).astype(np.float32),
                   (rng.randn(HID, C, DEG2+1)*0.1).astype(np.float32) ] + \
    ...
    def evaluate():
        ...
        Aa = np.tanh(Hh)
        Z = np.einsum("nhd,hcd->nc", cheb_T(Aa, DEG2), C2)
    ...
    pd.DataFrame([{"modello": f"KAN ml num10+cat4 ({HID}h, deg2={DEG2})", "macro_f1": round(mac,4),
                   "weighted_f1": round(wgt,4), "parametri": n_par,
                   "riferimento_lgbm": 0.9675}]).to_csv(_os.environ.get("MLCAT_OUT", "results/kan_ml_cat_mc_real.csv"), index=False, lineterminator="\n")
    pickle.dump(st, open(CK, "wb"))
    print("DONE")
