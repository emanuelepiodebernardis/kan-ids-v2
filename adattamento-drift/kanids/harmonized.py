...
# ─────────────────────────────────────────────────────────────
# SPAZIO RIDOTTO — il massimo comune denominatore fra TRE dataset
# ─────────────────────────────────────────────────────────────
...
# connessione. Le sue 47 feature sono durata, statistiche sulla dimensione
# dei pacchetti, tassi e conteggi di flag TCP.
#
# Sette delle tredici feature numeriche ricche dipendono dalla direzione
# (le due asimmetrie, i due payload medi, i quattro conteggi per direzione)
# e vanno perse. Restano sei quantita' che i tre dataset misurano davvero.
#
# NON si ricostruiscono le direzioni da Srate/Drate: sarebbe una stima
# spacciata per misura, e in CIC-IoT-2023 Drate e' quasi sempre zero.
# Meglio uno spazio piu' povero ma onesto, con accanto la misura di quanto
# costa impoverirlo (scripts/spazio_ridotto.py).
RIDOTTO_NUMERIC = [
...
# CIC-IoT-2023 non ha lo stato della connessione, ma ha i conteggi dei flag
# TCP. Lo stato si ricostruisce dalla stessa nozione semantica usata per
# Zeek e Argus: chi ha chiuso, chi ha resettato, chi non ha mai risposto.
# E' una decisione a priori come le altre due, non tarata sui dati.
def _state_da_flag(df: pd.DataFrame) -> np.ndarray:
    ...
    out[fin > 0] = "closed"
    out[rst > 0] = "reset"
    # senza TCP non esiste una nozione di stato: UDP e ICMP finiscono in
    # "incomplete", che e' dove Argus mette i flood senza risposta
...
def _cic_label(df: pd.DataFrame) -> np.ndarray:
    """0/1 dalla colonna label della famiglia CIC.

    load_cic() la normalizza gia' a 0/1 prima di qui; questa funzione resta
    robusta anche se viene chiamata su un dataframe grezzo non passato da
    load_cic (etichetta ancora stringa multiclasse tipo "BenignTraffic").
    """
...
def build_ridotto_da_ricco(h: pd.DataFrame) -> pd.DataFrame:
    """Proietta lo spazio ricco (TON_IoT, BoT-IoT) su quello ridotto.

    Serve al controllo: le stesse righe, gli stessi modelli, solo meno
    colonne. La differenza di risultato e' allora attribuibile alla
    riduzione dello spazio e a nient'altro.
    """
...
# ─────────────────────────────────────────────────────────────
# SPAZIO MINIMO — cio' che sopravvive con la famiglia CIC
# ─────────────────────────────────────────────────────────────
# I CSV della famiglia CIC-IoT/CIC-IoMT hanno 45 colonne e nessuna durata
# utilizzabile:
#   - "Duration" e' il TTL del pacchetto (mediana 64, massimo 248), non un
#     tempo di flusso;
#   - "IAT" e' corrotto: mescola timestamp Unix assoluti (169470306, cioe'
#     14 settembre 2023) con veri tempi di interarrivo (0,091).
# Senza durata cadono anche flow_rate e byte_rate. Restano TRE numeriche.
#
# C'e' di piu', e va detto prima dei numeri: quelle righe non sono flussi
# bidirezionali ma finestre scorrevoli di pacchetti. Trasferire fra questo
# dataset e TON_IoT/BoT-IoT/UNSW-NB15 non e' solo cambiare dominio, e'
# cambiare unita' di osservazione.
MINIMO_NUMERIC = ["bytes_total", "pkts_total", "payload_mean"]
MINIMO_SKEWED = set(MINIMO_NUMERIC)


def build_minimo_da_ricco(h: pd.DataFrame) -> pd.DataFrame:
    """Proietta lo spazio ricco sulle tre numeriche comuni con la famiglia CIC."""
    b, p = h["bytes_total"].to_numpy(), h["pkts_total"].to_numpy()
    out = pd.DataFrame({"bytes_total": b, "pkts_total": p,
                        "payload_mean": b / (p + 1.0)})
    out["proto_h"] = h["proto_h"].to_numpy()
    out["state_h"] = h["state_h"].to_numpy()
    out["label"] = h["label"].to_numpy()
    return out
...
def build_minimo_cic(df: pd.DataFrame) -> pd.DataFrame:
    """CIC-IoMT/IoT (45 colonne) -> spazio minimo.

      bytes_total  <- Tot sum   (somma delle dimensioni nella finestra)
      pkts_total   <- Number    (numero di pacchetti nella finestra)
      payload_mean <- AVG       (dimensione media)
      proto_h      <- indicatori TCP/UDP/ICMP/ARP
      state_h      <- conteggi dei flag TCP
      label        <- gia' impostata dal caricatore, dal nome del file
    """
    def col(*nomi):
        for n in nomi:
            if n in df.columns:
                return pd.to_numeric(df[n], errors="coerce").fillna(0.0).to_numpy(np.float64)
        raise KeyError(f"nessuna di {nomi} presente")

    b, p = col("Tot sum"), col("Number")
    out = pd.DataFrame({"bytes_total": b, "pkts_total": p,
                        "payload_mean": col("AVG")})
