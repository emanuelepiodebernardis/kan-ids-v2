"""Frozen prediction replay and RUN-once guard; no hardware RAM simulation claim."""
import os
...
class FirmwareHost(unittest.TestCase):
 def test_all_ten_target_model_paths(self):
  ...
  with tempfile.TemporaryDirectory(prefix='ram500_') as tmp:
   for board in ['MEGA','C3']:
    for variant,flag in VARIANTS.items():
     with self.subTest(board=board,variant=variant):
      ...
      self.assertIn('HOST_RAM_PASS',run.stdout)
