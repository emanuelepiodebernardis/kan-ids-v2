...
ENV = "mega_fnb58_kanml_pilot"
PROTOCOL = "kanids-mega-fnb58-kanml-pilot-v1"
...
def validate_ready(lines: list[str]) -> dict:
    ...
    if tags["MODEL"] != {"variant": "ml_coeff_int8", "model_bytes": "5244", "cohort_sha256": COHORT_SHA}:
...
def main(argv=None) -> int:
    ...
    run_id = datetime.now(timezone.utc).strftime("MEGA_KANML_PILOT_%Y%m%dT%H%M%SZ_") + uuid.uuid4().hex[:8]
    ...
    try:
        ...
        if args.build_only:
            ...
            print("Имя файла: mega_kanml_pilot_01.cfn. Энергию ещё нужно проверить по этому файлу.")
        ...
        print("Пришлите этот ZIP; после опыта также приложите mega_kanml_pilot_01.cfn.")
    return code
