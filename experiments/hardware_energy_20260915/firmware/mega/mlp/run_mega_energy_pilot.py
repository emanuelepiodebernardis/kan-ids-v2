...
ENV = "mega_fnb58_mlp_pilot"
PROTOCOL = "kanids-mega-fnb58-mlp-pilot-v1"
...
def validate_ready(lines: list[str]) -> dict:
    ...
    if tags["MODEL"] != {"variant": "mlp16_int8", "model_bytes": "760", "cohort_sha256": COHORT_SHA}:
...
def main(argv=None) -> int:
    ...
    run_id = datetime.now(timezone.utc).strftime("MEGA_MLP_PILOT_%Y%m%dT%H%M%SZ_") + uuid.uuid4().hex[:8]
    ...
    try:
        ...
        if args.build_only:
            ...
            print("Имя файла: mega_mlp_pilot_01.cfn. Энергию ещё нужно проверить по этому файлу.")
        ...
        print("Пришлите этот ZIP; после опыта также приложите mega_mlp_pilot_01.cfn.")
    return code
