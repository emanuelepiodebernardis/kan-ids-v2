 ...
 * Frozen MLP16 model and first 20 common-cohort prepared inputs.
...
#if defined(HB_COEFF) || defined(HB_MLCOEFF) || defined(HB_LUT14) || defined(HB_DT5)
#error "This acquisition pilot requires only HB_MLP"
#endif
#ifndef HB_MLP
#define HB_MLP
#endif
#include "hardware_cohort_select.h"
static_assert(HB_MODEL_BYTES == 760, "Frozen MLP16 model byte count changed");
static_assert(MLP16_HID == 16, "Frozen MLP hidden width changed");
...
extern "C" __attribute__((noinline)) uint32_t pilot_active_batch(uint32_t count) {
  ...
  while (count--) {
    ...
    checksum += mlp16_predict(inputs[row], categories[row]);
...
void setup() {
  ...
  Serial.begin(115200);
  delay(1500);
  Serial.println(F("HELLO protocol=kanids-mega-fnb58-mlp-pilot-v1"));
#ifdef HOST_CHECK
  Serial.println(F("EXECUTION mode=host_simulation hardware_measurement=0"));
#endif
  Serial.print(F("MODEL variant=mlp16_int8 model_bytes="));
  ...
  Serial.println();
  for (uint8_t i = 0; i < INPUT_COUNT; ++i) {
    const uint8_t predicted = mlp16_predict(inputs[i], categories[i]);
  ...
  for (uint8_t i = 0; i < 64; ++i) {
    __asm__ __volatile__("" ::: "memory");
    warm_checksum += mlp16_predict(inputs[warm_row], categories[warm_row]);
