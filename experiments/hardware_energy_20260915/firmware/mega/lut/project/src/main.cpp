 ...
 * Frozen sampled-LUT model and first 20 common-cohort prepared inputs.
...
#if defined(HB_COEFF) || defined(HB_MLCOEFF) || defined(HB_MLP) || defined(HB_DT5)
#error "This acquisition pilot requires only HB_LUT14"
#endif
#ifndef HB_LUT14
#define HB_LUT14
#endif
#include "hardware_cohort_select.h"
static_assert(HB_MODEL_BYTES == 20554, "Frozen LUT model byte count changed");
static_assert(KLUT_L == 1025, "Frozen LUT grid length changed");
...
extern "C" __attribute__((noinline)) uint32_t pilot_active_batch(uint32_t count) {
  ...
  while (count--) {
    ...
    checksum += kan14_lut_predict(inputs[row], categories[row]);
...
void setup() {
  ...
  Serial.begin(115200);
  delay(1500);
  Serial.println(F("HELLO protocol=kanids-mega-fnb58-lut-pilot-v1"));
#ifdef HOST_CHECK
  Serial.println(F("EXECUTION mode=host_simulation hardware_measurement=0"));
#endif
  Serial.print(F("MODEL variant=lut_int16 model_bytes="));
  ...
  Serial.println();
  for (uint8_t i = 0; i < INPUT_COUNT; ++i) {
    const uint8_t predicted = kan14_lut_predict(inputs[i], categories[i]);
  ...
  for (uint8_t i = 0; i < 64; ++i) {
    __asm__ __volatile__("" ::: "memory");
    warm_checksum += kan14_lut_predict(inputs[warm_row], categories[warm_row]);
