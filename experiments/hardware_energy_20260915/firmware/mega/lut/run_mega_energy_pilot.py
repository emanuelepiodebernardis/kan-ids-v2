...
ENV = "mega_fnb58_lut_pilot"
PROTOCOL = "kanids-mega-fnb58-lut-pilot-v1"
...
def validate_ready(lines: list[str]) -> dict:
    ...
    if tags["MODEL"] != {"variant": "lut_int16", "model_bytes": "20554", "cohort_sha256": COHORT_SHA}:
...
def main(argv=None) -> int:
    ...
    run_id = datetime.now(timezone.utc).strftime("MEGA_LUT_PILOT_%Y%m%dT%H%M%SZ_") + uuid.uuid4().hex[:8]
    ...
    try:
        ...
        if args.build_only:
            ...
            print("Имя файла: mega_lut_pilot_01.cfn. Энергию ещё нужно проверить по этому файлу.")
        ...
        print("Пришлите этот ZIP; после опыта также приложите mega_lut_pilot_01.cfn.")
    return code
