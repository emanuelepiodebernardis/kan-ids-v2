 ...
 * Frozen DT5 decision tree model and first 20 common-cohort prepared inputs.
...
#if defined(HB_COEFF) || defined(HB_MLP) || defined(HB_LUT14) || defined(HB_MLCOEFF)
#error "This acquisition pilot requires only HB_DT5"
#endif
#ifndef HB_DT5
#define HB_DT5
#endif
#include "hardware_cohort_select.h"
static_assert(HB_MODEL_BYTES == 285, "Frozen DT5 decision tree model byte count changed");
static_assert(DT5_NFEAT == 14, "Frozen DT5 feature count changed");
static_assert(DT5_NNODE == 57, "Frozen DT5 node count changed");
...
/* DT5-only ceiling: 120000000 * 200 / 1000 = 24000000. The minimum
 * valid calibration is unchanged; no selected count is silently clipped.
 * Both count and binary-prediction checksum fit uint32_t at this ceiling. */
static const uint32_t MAX_COUNT = 24000000UL;
static_assert(MAX_COUNT <= UINT32_MAX, "Batch count must fit uint32_t");
static int16_t inputs[INPUT_COUNT][14];
...
extern "C" __attribute__((noinline)) uint32_t pilot_active_batch(uint32_t count) {
  ...
  while (count--) {
    ...
    checksum += dt5_predict(inputs[row]);
...
void setup() {
  ...
  Serial.begin(115200);
  delay(1500);
  Serial.println(F("HELLO protocol=kanids-mega-fnb58-dt5-pilot-v1"));
#ifdef HOST_CHECK
  Serial.println(F("EXECUTION mode=host_simulation hardware_measurement=0"));
#endif
  Serial.print(F("MODEL variant=dt5 model_bytes="));
  ...
  Serial.println();
  for (uint8_t i = 0; i < INPUT_COUNT; ++i) {
    const uint8_t predicted = dt5_predict(inputs[i]);
  ...
  for (uint8_t i = 0; i < 64; ++i) {
    __asm__ __volatile__("" ::: "memory");
    warm_checksum += dt5_predict(inputs[warm_row]);
