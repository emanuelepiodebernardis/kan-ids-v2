...
ENV = "mega_fnb58_dt5_pilot"
PROTOCOL = "kanids-mega-fnb58-dt5-pilot-v1"
...
def validate_ready(lines: list[str]) -> dict:
    ...
    if tags["MODEL"] != {"variant": "dt5", "model_bytes": "285", "cohort_sha256": COHORT_SHA}:
    ...
    if not (ready["target_us"] == 120000000 and ready["cal_n"] == 200
            ...
            and 20 <= ready["count"] <= 24000000 and ready["count"] % 20 == 0):
...
def run_command(command: list[str], run_dir: Path, stem: str, record: dict) -> None:
    ...
    info = {"argv": command, "started_utc": utc(), "log": stem + ".log",
            "child_python_io_encoding": child_env["PYTHONIOENCODING"]}
    record["commands"].append(info)
    print("\n> " + subprocess.list2cmdline(command), flush=True)
    with (run_dir / (stem + ".log")).open("w", encoding="utf-8", newline="\n") as handle:
        handle.write("# child PYTHONIOENCODING=utf-8\n")
        handle.flush()
...
def main(argv=None) -> int:
    ...
    run_id = datetime.now(timezone.utc).strftime("MEGA_DT5_PILOT_%Y%m%dT%H%M%SZ_") + uuid.uuid4().hex[:8]
    ...
    try:
        ...
        if args.build_only:
            ...
            print("Имя файла: mega_dt5_pilot_01.cfn. Энергию ещё нужно проверить по этому файлу.")
        ...
        print("Пришлите этот ZIP; после опыта также приложите mega_dt5_pilot_01.cfn.")
    return code
